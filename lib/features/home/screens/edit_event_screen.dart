...
import 'package:gdsc_atmiya/bloc/event/edit/edit_event_bloc.dart';
...
class _EditEventScreenState extends State<EditEventScreen> {
  ...
  void initState() {
    ...
    speakers = widget.event.speakers.map((e) => e.toMap()).toList();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocListener<PublishEventBloc, PublishEventState>(
        ...
        child: BlocProvider(
          create: (context) => EditEventBloc(widget.event),
          child: Builder(
            builder: (context) {
              final eventDetailsBloc = context.read<EditEventBloc>();

              return FormBlocListener<EditEventBloc, Map<String, dynamic>,
                  String>(
                onSubmitting: (context, state) {},
                onSuccess: (context, state) {
                  dev.log(
                    'Editing event: ${state.successResponse}',
                    name: 'Event',
                  );
                  final eventData = state.successResponse!;
                  if (widget.event.eventThumbnailUrl != null) {
                    eventData['eventThumbnailUrl'] = widget.event.eventThumbnailUrl;
                  }

                  if (widget.event.eventBannerUrl != null) {
                    eventData['eventBannerUrl'] = widget.event.eventBannerUrl;
                  }
                ...
                child: BlocBuilder<EditEventBloc, FormBlocState>(
                    ...
                    if (state is FormBlocLoading) {
                      ...
                      return Center(
                        child: ElevatedButton(
                          onPressed: context.read<EditEventBloc>().reload,
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              image: speakerData['image'] != null
                  ...
                  : speakerData['photoUrl'] != null
                      ? DecorationImage(
                          image: NetworkImage(speakerData['photoUrl']),
                          fit: BoxFit.cover,
                        )
                      : null,
            ),
            ...
            child: Container(
              decoration: BoxDecoration(
                image: (bannerFile != null)
                    ...
                    : (widget.event.eventBannerUrl != null)
                        ? DecorationImage(
                            image: NetworkImage(
                              widget.event.eventBannerUrl!,
                            ),
                            fit: BoxFit.cover,
                          )
                        : const DecorationImage(
            ...
            child: Container(
              decoration: BoxDecoration(
                image: (thumbnailFile != null)
                    ...
                    : (widget.event.eventThumbnailUrl != null)
                        ? DecorationImage(
                            image: NetworkImage(
                              widget.event.eventThumbnailUrl!,
                            ),
                            fit: BoxFit.cover,
                          )
                        : const DecorationImage(
